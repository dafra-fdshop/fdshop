...
?>
	<tr style="vertical-align: top;">
		...
		<td style="text-align: right;" class="priceCol" ><?php echo '<span >'.$this->currency->priceDisplay($item->product_item_price, $this->user_currency_id) .'</span><br />'; ?>
		</td>
...
<?php } ?>
    ...
    <!-- Frühbucherrabatt wird wieder gelöscht-->
   <?php
	$rawPrice = $this->orderdetails['details']['BT']->order_salesPrice;
	$discountPriceEdit = round($rawPrice * 0.1, 2);
	
	$orderDatefrue = $this->orderdetails['details']['BT']->order_created;
 	
	if (strtotime($orderDatefrue) > strtotime('2025-03-30')) {
    ?>
    
    <tr class="sectiontableentry1">
		<td colspan="5" style="text-align: right;"><?php echo vmText::_('COM_VIRTUEMART_PRODUCT_FRUERABATT'); ?></td>
		<td style="text-align: right;">&nbsp;</td>
		<td style="text-align: right;"><?php echo vmText::_('-') . ' ' . $this->currency->priceDisplay($discountPriceEdit, $this->user_currency_id); ?></td>
	</tr>
	<?php
	}
...
foreach($this->orderdetails['calc_rules'] as $rule){
	...
	$discountPriceTaxOrder = round($rawPrice / 1.19 * 0.19, 2);
   ?> 
    <tr>
		<td style="text-align: right;" class="pricePad" colspan="5"><?php echo vmText::_('enthaltene MwSt') ?></td>
		<td style="text-align: right;">&nbsp;</td>
		<?php if ( VmConfig::get('show_tax')) {  ?>
		<td style="text-align: right;"><span  class='priceColor2'><?php echo $this->currency->priceDisplay($discountPriceTaxOrder, $this->user_currency_id); ?></span></td>
		<?php } ?>
	</tr>

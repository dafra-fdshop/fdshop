...
if (
    ...
    if ($fdAutoCouponEnabled && $fdAutoCouponLabel !== '') {
        $couponText = $fdAutoCouponLabel;
    } else {
        $couponText = vmText::_('COM_VIRTUEMART_COUPON_DISCOUNT');

        if (!empty($this->orderDetails['details']['BT']->coupon_code)) {
            $couponText .= ': ' . $this->orderDetails['details']['BT']->coupon_code;
        }
    }

    echo $couponText . ' ' . vmText::_('COM_VIRTUEMART_PRICE') . ': '
        . $this->currency->priceDisplay($this->orderDetails['details']['BT']->coupon_discount, $this->user_currency_id);
    echo "\n";
}

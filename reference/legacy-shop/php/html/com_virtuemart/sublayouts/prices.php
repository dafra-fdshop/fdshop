...
<div class="product-price" id="productPrice<?php echo $product->virtuemart_product_id ?>" data-vm="product-prices">
	...
	// --- Hauptpreis + Info (MwSt/Delivery) als kontrollierbarer Row-Block ---
$salesPriceHtml = $currency->createPriceDiv('salesPrice', 'COM_VIRTUEMART_PRODUCT_SALESPRICE', $product->prices);
$salesPriceTtHtml = $currency->createPriceDiv('salesPriceTt', 'COM_VIRTUEMART_PRODUCT_SALESPRICE_TT', $product->prices);

$infotax = vmConfig::get('vm_prices_info_tax', 0);
$infodelivery = vmConfig::get('vm_prices_info_delivery', 0);

$pricesInfoHtml = '';
if ($infotax == 1 || $infodelivery == 1) {
	ob_start();
	?>
	<div class="vm-prices-info">
		<?php
		if ($infotax == 1) {
			$taxclusive = ($product->prices['priceWithoutTax'] == $product->prices['salesPrice'])
				? 'COM_VM_TAX_EXCLUSIVE'
				: 'COM_VM_TAX_INCLUSIVE';
	...
	<?php
	$pricesInfoHtml = trim(ob_get_clean());
}

// Ausgabe: 1 Zeile (Preis links, Info rechts unten bündig)
echo '<div class="fd-price-row">';
echo $salesPriceHtml;
echo $pricesInfoHtml;
echo '</div>';

// TT wie gehabt (kannst du später auch scopen/ausblenden je nach Bereich)
echo $salesPriceTtHtml;
	
	//Frühbucher wieder Löschen wenn Aktion vorbei
	//$rawPrice = $product->prices['salesPrice'];
	//$discountPrice = round($rawPrice * 0.9, 2);
	//ende Frühbucher
	...
	//Frühbucher wieder Löschen wenn Aktion vorbei
	//echo '<span class="discount-price">'. vmText::_('COM_VIRTUEMART_PRODUCT_FRUEPREIS') . $discountPrice . vmText::_('COM_VIRTUEMART_PRODUCT_ZEICHEN') . '</span>';
	//ende Frühbucher
	//Frühbucher dieser teil ist nur wege nder anzeige
	echo '<span class="discount-price">'. vmText::_('') . $discountPrice . vmText::_('') . '</span>';
	//ende Frühbucher

...
[[ "$(sql "SELECT COUNT(*) FROM ${JOOMLA_DB_PREFIX}modules AS m INNER JOIN ${JOOMLA_DB_PREFIX}modules_menu AS mm ON mm.moduleid=m.id WHERE m.client_id=0 AND m.module='mod_fdshop_filter' AND m.published=1 AND m.position='sidebar-left' AND mm.menuid=900900;")" == 1 ]] || fail 'frontend filter module missing or not assigned'
...
[[ "$(sql "SELECT COUNT(*) FROM ${JOOMLA_DB_PREFIX}fdshop_media WHERE product_id=900100 AND media_type='youtube';")" == 3 ]] || fail 'product videos missing'

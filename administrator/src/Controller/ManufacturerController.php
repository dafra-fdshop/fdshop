...
class ManufacturerController extends FormController
{
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        if (!Factory::getApplication()->getIdentity()->authorise($action, 'com_fdshop')) {
            $this->setMessage('Sie sind nicht berechtigt, diesen Hersteller zu speichern.', 'error');
            $this->setRedirect(
                $id > 0
                    ? 'index.php?option=com_fdshop&view=manufacturer&layout=edit&id=' . $id
                    : 'index.php?option=com_fdshop&view=manufacturers'
            );
        ...
        if ($validData === false) {
            Factory::getApplication()->setUserState('com_fdshop.edit.manufacturer.data', $data);
            $this->setMessage($model->getError() ?: 'Die Formulardaten sind ungültig.', 'error');
            $this->setRedirect(
                'index.php?option=com_fdshop&view=manufacturer&layout=edit' . ($id > 0 ? '&id=' . $id : '')
            );
        ...
        Factory::getApplication()->setUserState('com_fdshop.edit.manufacturer.data', null);
